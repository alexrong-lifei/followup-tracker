...
def compute_action_today(row, today=None):
    ...
    needs_action = False
    if next_follow and next_follow <= today:
        needs_action = True
    if due_date and due_date <= today and "alex" in responsible:
        needs_action = True
...
def refresh_computed_fields(df: pd.DataFrame, today=None) -> pd.DataFrame:
    ...
    if "Days Since Last Follow-Up" in df.columns:
        def calc_days(v):
            d = coerce_date(v)
            return (today - d).days if d else ""
        df["Days Since Last Follow-Up"] = df["Last Follow-Up"].apply(calc_days)

    if "Action Today?" in df.columns:
...
def format_tracker_for_export(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out = refresh_computed_fields(out)
    for col in DATE_COLUMNS:
...
def auto_style_tracker_sheet(ws, raw_df: pd.DataFrame):
    ...
    # apply highlighting based on raw_df row order
...
with tab1:
    ...
    c2.metric("Overdue", int(sum(
        (today_df["Next Follow-Up"].apply(lambda x: coerce_date(x) or date.max)) < date.today()
    )))
    ...
    if len(df) == 0:
        ...
        with st.form("edit_task_form"):
            ...
            with col1:
                ...
                priority = st.selectbox("Priority", PRIORITY_OPTIONS, index=PRIORITY_OPTIONS.index(row["Priority"]) if row["Priority"] in PRIORITY_OPTIONS else 0)
            with col2:
                status = st.selectbox("Status", STATUS_OPTIONS, index=STATUS_OPTIONS.index(row["Status"]) if row["Status"] in STATUS_OPTIONS else 0)
...
with tab4:
    ...
    st.code("streamlit run followup_tracker_app.py", language="bash")
    st.caption("把这个 .py 文件保存后，在本地终端运行上面的命令即可打开网页版。")
